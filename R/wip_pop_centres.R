...
intermediate <- inner_join(data,edata,by=c("LSOA11CD"="geography code"))%>%
  ...
  mutate(`Weighted emissions`= Total*flat_population,
         ...
         percentage=flat_population/`Ethnic group: Total: All usual residents`) %>%

  group_by(`Ethnic group`) %>%

  mutate(conventional_name=ntile(percentage,5))
...
stitched_shapefile <- inner_join(intermediate,raw_shapefile, by="LSOA11CD") %>%
  dplyr::select(`Ethnic group`,LSOA11CD,geometry,conventional_name) %>%
  mutate(selector=case_when(conventional_name<2.1~2,
                            conventional_name>2~1),
         NOx_selector=)
