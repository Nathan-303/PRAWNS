...
#'   year=2019
#'   )
facet_ethnicity_quintile_dual_y_src <- function(prawn_path,pollutant,year){
...
weighted <- plottable %>% group_by(IMD,caption) %>%
  summarise(noxpop=sum(Total)*50/(nrow(plottable)))
