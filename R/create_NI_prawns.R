...
create_NI_prawns <- function(raster_path=FALSE,
                          tif_path=FALSE,
...
#Three chained if statements, which trigger if there is a path to the appropriate file in the function call
  ...
  #Read the shapefile
  SOA_shapefile <- vect(shapefile_path)

  #Calculate the average for each polygon in the shapefile
  index <-  c(1:length(SOA_shapefile))
  transient <- sf::st_as_sf(SOA_shapefile[index])
  ...
  output <- tibble(poll_mean=pollution_mean,
                   SOA2001=SOA_shapefile$SOA_CODE
                   ) %>% unnest(poll_mean)
