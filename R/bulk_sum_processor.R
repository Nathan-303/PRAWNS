...
bulk_sum_processor <- function(raster_path,
                           shapefile_path,
                           pollutant_data_name,
                           year,
                           pollutant,
...
proc_tag <- paste0(pollutant,"_emissions_sum_in_",year,"_v",iteration)
...
for ( index in c(1:3)){
  ...
  if (index==2){
    proc_tag <- paste0(pollutant,"_emissions_sum_in_",year,"_v",iteration,"/Londonless")
    prawn_path <- paste0(proc_tag,"/PRAWN.csv")
    #create the folder that everything goes in
    dir.create(path=paste0(proc_tag))
  ...
  if (index==3){
    proc_tag <- paste0(pollutant,"_emissions_sum_in_",year,"_v",iteration,"/na is 0")
    prawn_path <- paste0(proc_tag,"/PRAWN.csv")
    #create the folder that everything goes in
    dir.create(path=paste0(proc_tag))
