...
facet_ethnicity_deprivation <- function(prawn_path,pollutant,year){
...
boxxy <- plottable %>% group_by(`Ethnic group`,tile) %>% summarise(maxper=max(Percentage),
...
plottable2 <- plottable %>% inner_join(boxxy,join_by(`Ethnic group`,tile))
